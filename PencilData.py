...
class Pencil_Data(object):
	...
	#   7/27/2019  added orbital energy for eccentric cases 
	    ...
	    # if temperature is too be calculated
	        ...
	        TTm_rate	= np.mean(np.gradient(TTm[:]))
                ...
                while D <= len(GlobalTemp_Mean)-2:
                    DGlobalTemp_Mean.append(GlobalTemp_Mean[D+1]-GlobalTemp_Mean[D])
                    D=D+dD
	    ...
	    mstar	    = 8.35e2
	    twave           = mstar * aspect_ratio**4 / (q * Sigmap * semi_major**2 *  Omegap)
 	    ...
 	    # for convience in calculating the longitude of the perihelion
            ...
            if ecc.all()    == 0:
                ...
                while i     <= len(ecc)-1:
                    ...
                    true_angle.append((1.0/fixed_ecc[i])*((sma[i]*(1.0-fixed_ecc[i]**2.0)/radius[i])-1.0))
            ...
            long_perihelion = np.absolute(Omega*(180/np.pi))+true_anomaly    #arguement of perihelion
            long_perihelion = long_perihelion*(np.pi/180) # turn the arguement of perihelion into radians so that we can compare it to other papers
		...
		try:
			while Standard_Orbit <= Max_Orbits-Dynamic:
				...
				if Calc_Temp == True:
					fv              = pc.read_var(trimall=True,ivar=Standard_Orbit,magic=['TT'])
			                rho_fv          = fv.rho
                			temp_fv         = fv.TT
		        	        shock_fv        = fv.shock
	    ...
	    #======================================
		    ...
		    # fits
                    ...
                    while i <= len(OE_time)-1:
                        OE_fit.append(OE_Grad[i]*np.sin(OE_Sum[i]+OE_time[i])+OE_Sum[0])
                        i=i+di
            ...
            twave           = Mstar * aspect_ratio**4 / (q*Sigma * semi_major**2 * Omegap)
            #eh              = eccentricity/aspect_ratio
            ...
            Mu_coef = 0.0
            for k in range(Num):
                Mu_coef     += w[k]*self.fourier_hermite_function((Z[k]),radial_coord,1,I)
            ...
            Mu_coef2 = 0.0
            for k in range(Num):
                Mu_coef2     += w[k]*self.fourier_hermite_function((Z[k]),radial_coord,Mu_coef,I)
            ...
            Mu_coef3 = 0.0
            for k in range(Num):
                Mu_coef3     += w[k]*self.fourier_hermite_function((Z[k]),radial_coord,Mu_coef2,I)
