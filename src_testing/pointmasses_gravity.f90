...
!
    ...
    ! don't want to output personal debug tactics
    logical :: ldebug=.false.

    ! aquire the acceleration due to the disk

    call get_acceleration(acceleration)

    if (ldebug) then
      print*,''
      print*,'acceleration',acceleration(nx,ny,nz,2)
      print*,''
    endif
...
!
      ...
      ! coded: joshua 1/9/2019
      ! add acceleration from poission module if selfgravity is on
      real, dimension (nx,ny,nz,3) :: acceleration
      logical :: ldebug=.true.
!
      intent(out) :: accg
