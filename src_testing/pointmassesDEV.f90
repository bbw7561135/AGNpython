...
!
      ...
      if (lselfgravity) then
              ...
              print*,'===WORKS==='
              print*,'===WORKS==='
	      print*,'self gravity is on indeed!'
              print*,'===WORKS==='
              print*,'===WORKS==='
              print*,'===WORKS==='
              print*,'===WORKS==='
...
!***********************************************************************
    subroutine calc_poisson_selfgravity(selfgrav)
! 
!   adds acceleration of disk selfgravity to pointmasses besides the primary
!
!   1/8/2020: joshua /adapted from possion_logspirals (vince),
!	      and integrate_selfgravity modules (wlad)
!
!   seperated this subroutine from other similar ones to reduce errors from coupling 
!   well developed subroutines.
!
    use Poisson
      real, dimension (nx,ny,nz,3) :: acceleration
      real, dimension (nx),intent(inout) :: selfgrav
    ! don't want to output personal debug tactics with print
      logical :: ldebug=.true.

    ! aquire the acceleration due to the disk (vince's subroutine)
    call get_acceleration(acceleration)

    selfgrav=-1*(acceleration(nx,ny,nz,2))

    if (ldebug) then 
      !print*,'self grav',acceleration(nx,ny,nz,1)
      !WRITE(*,*) size(acceleration,2)
    endif

    endsubroutine calc_poisson_selfgravity
!***********************************************************************
...
!
     ...
     !joshua coded: check if add_gravity_module works or not

     ! call add_selfgravity_module()
!      
...
!
      !
      ! joshua 1/9/2019:
      ! updated subroutine to consider the acceleration from poission
      !
      if (lselfgravity) then
        call calc_poisson_selfgravity(selfgrav)
        !selfgrav = -1*(GNewton*density_scale*&
        !     density*jac*dv*(rrp**2 + rp0**2)**(-1.5))
      else
        selfgrav = -1*(GNewton*density_scale*&
             density*jac*dv*(rrp**2 + rp0**2)**(-1.5))
